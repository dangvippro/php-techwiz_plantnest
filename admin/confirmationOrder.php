...
<body>
    ...
    $result = $order->showConfirmationOrder($conn);
    
    //Update
    if(isset($_GET['action']) && $_GET['action'] == "confirm"){
        $orderID = $_GET['id'];
        if ($order->setConfirmation($conn, $orderID)) {
            echo '<div style="margin: 1% 0%;"><i class="fas fa-check-circle"></i><strong> Confirmed!</strong></div>';
            header('location: confirmationOrder.php');
        } else {
            echo '<div>Confirm error!</div>';
        }
    }
    ...
    if (isset($_GET['action']) && ($_GET['action'] == 'deny')) {
        $orderID = $_GET['id'];
        if ($order->denyConfirmation($conn, $orderID)) {
            echo '<div style="margin: 1% 0%;"><i class="fas fa-check-circle"></i><strong> Denied!</strong></div>';
            header('location: confirmationOrder.php');
        } else {
            echo '<div>Deny error!</div>';
        }
    }
           
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $result = $order->findConfirmationOrders($conn, $search);
    }
        ...
        <div class="body-container" style="width: 90%;">
            <div class="body-container-nav">
                <div class="body-container-refresh">
                    <a type="button" class="btn btn-outline-dark" href="confirmationOrder.php">
                        <i class="fa-solid fa-arrows-rotate"></i> Refresh
                    </a>
                </div>
                <div class="d-flex">
                    <form class="d-flex search" method="get" style="margin-right: 20px;">
                        <input type="form-control" style="border: none; padding: 6px 12px; border: 1px solid #ddd; outline: none;" name="search" id="search" placeholder="Enter order date" required>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <a class="body-container-edit" href="?action=confirm&id=<?php echo $row["orderID"] ?>">
                            <i class="fa-solid fa-check" style="color: #000;"></i>
                            </a>
                        </td>
                        <td style="text-align: center;">
                            <a class="body-container-delete" href="?action=deny&id=<?php echo $row["orderID"] ?>">
