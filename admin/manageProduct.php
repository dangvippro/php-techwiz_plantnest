...
<body>
    ...
    $result = $product->showAll($conn);

    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $result = $product->findElementByName($conn, $search);
    }

    if (isset($_GET['action']) && $_GET['action'] == "delete") {
        $productID = $_GET['productID'];
        $product->delete($conn, $productID);
        ...
        <div class="body-container" style="width: 90%;">
            <div class="body-container-nav">
                <div class="body-container-refresh">
                    ...
                    <?php $count = $product->countProduct($conn);
                    if($count->num_rows > 0) {
                        while ($row = $count->fetch_assoc()) {
                    ?>
                    <span class="btn btn-outline-dark">Total number of products: <?php echo $row['count(*)'] ?></span>
                    <?php } 
                    } ?>
                </div>
                <div class="d-flex">
                    ...
                    <a type="button" class="btn btn-outline-dark" href="./addProducts.php">
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td><?php echo $row["growthHabits"] ?></td>
                                <td><?php echo $row["light"] ?></td>
                                <td><?php echo $row["water"] ?></td>
                                <td><?php echo $row["categoryID"] ?></td>
                                <td><?php echo $row["unitsInStock"] ?></td>
                                <td style="text-align: center;">
                                    <a class="body-container-edit" href="./editProduct.php?action=editPro&productID=<?php echo $row["productID"] ?>">
                                    <i class="fa-solid fa-pen-to-square" style="color: #000000;"></i>
                                    </a>
                                </td>
                                <td style="text-align: center;">
                                    <a class="body-container-delete" href="manageProduct.php?action=delete&productID=<?php echo $row["productID"] ?>">
