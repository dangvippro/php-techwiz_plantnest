...
<body>
    ...
    $result = $userReview->showAll($conn);
    if(isset($_GET['action']) && $_GET['action'] == "seen"){
        $reviewNo = $_GET['reviewNo'];
        $update = $userReview->updateSeen($conn, $reviewNo);
        if($update){
            echo '<div style="margin: 1% 0%;"><i class="fas fa-check-circle"></i><strong> Update successfully!</strong></div>';
    ...
    if(isset($_GET['action']) && $_GET['action'] == "deleteReview"){
        $reviewNo = $_GET['reviewNo'];
        $delete = $userReview->deleteReview($conn, $reviewNo);
        if($delete){
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $result = $userReview->findReview($conn, $search);
    }

//    if (isset($_GET['search'])) {
//        $keyword = $_GET['search'];
//        $result = $userReview->findReview($conn, $keyword);
//    }
        ...
        <div class="body-container" style="width: 90%;">
            <div class="body-container-nav">
                <div class="body-container-refresh">
                    <a type="button" class="btn btn-outline-dark" href="userReview.php">
                    ...
                    ?>
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <a class="body-container-edit" href="?action=seen&reviewNo=<?php echo $row["reviewNo"] ?>">
                                    <i class="fas fa-eye" style="color: #000000;"></i>
                                    </a>
                                </td>
                                <td style="text-align: center;">
                                    <a class="body-container-delete" href="?action=deleteReview&reviewNo=<?php echo $row["reviewNo"] ?>">
