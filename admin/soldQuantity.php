...
<body>
    ...
    $result = $orderdetails->showSoldQuantity($conn);

    if (isset($_GET['search'])) {
        $keyword = $_GET['search'];
        $result = $orderdetails->findSoldQuantity($conn, $keyword);
    }
        ...
        <div class="body-container" style="width: 90%;">
            <div class="body-container-nav">
                <div class="body-container-refresh">
                    <a type="button" class="btn btn-outline-dark" href="soldQuantity.php">
                        <i class="fa-solid fa-arrows-rotate"></i> Refresh
                    </a>
                    <?php $count = $orderdetails->countSoldQuantity($conn);
                    if($count->num_rows > 0) {
                        while ($row = $count->fetch_assoc()) {
                    ?>
                    <span class="btn btn-outline-dark">The number of products sold: <?php echo $row['sum(quantity)'] ?></span>
                    <?php } 
                    } ?>
                </div>
                <div class="d-flex">
                    <form class="d-flex search" method="get" style="margin-right: 20px;">
                        <input type="form-control" style="border: none; padding: 6px 12px; border: 1px solid #ddd; outline: none;" name="search" id="search" placeholder="Enter product name to search" required>
