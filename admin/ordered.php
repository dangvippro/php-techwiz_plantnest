...
<body>
    ...
    $result = $order->showOrdered($conn);

    //Update delivery
    if(isset($_GET['action']) && $_GET['action'] == "delivery"){
        $orderID = $_GET['id'];
        if ($order->setDelivery($conn, $orderID)) {
            echo '<div style="margin: 1% 0%;"><i class="fas fa-check-circle"></i><strong> Delivered!</strong></div>';
            header('location: ordered.php');
        } else {
            echo '<div>Delivery error!</div>';
        }
    }
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $result = $order->findOrdered($conn, $search);
    }
    ?>
        ...
        <div class="body-container" style="width: 90%;">
            <div class="body-container-nav">
                <div class="body-container-refresh">
                    <a type="button" class="btn btn-outline-dark" href="ordered.php">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <a class="body-container-edit" href="?action=delivery&id=<?php echo $row["orderID"] ?>">
                    ...
                    <?php
                        ...
                        echo 'No orders need to be shipped.';
                    }
                    ?>
                </tbody>
            </table>
